...
<div class="form-container" style="max-width:900px;">
    <h3 class="section-title" style="margin-bottom:24px;">
        <i class='bx bx-edit'></i> Edit Transaksi #<?= $trans['ID_Transaksi'] ?>
    </h3>

    <form action="/transaksi/update/<?= $trans['ID_Transaksi'] ?>" method="post" id="formTransaksi">
                    ...
                    <select name="ID_Kendaraan" id="selKendaraan" required>
                        ...
                        <?php foreach($kendaraan as $k): ?>
                            <option value="<?= $k['ID_Kendaraan'] ?>" 
                                ...
                                <?= $k['ID_Kendaraan'] == $trans['ID_Kendaraan'] ? 'selected' : '' ?>>
        ...
        <div class="section-block" style="margin-top:24px;">
            ...
            <div id="servisContainer">
                <p class="empty-hint" id="servisEmpty" style="color:var(--text-muted); font-size:13px; padding:12px; text-align:center; <?= !empty($detailServis) ? 'display:none;' : '' ?>">Belum ada servis ditambahkan (opsional)</p>
            </div>
        </div>
        ...
        <div class="section-block" style="margin-top:24px;">
            ...
            <div id="sparepartContainer">
                <p class="empty-hint" id="sparepartEmpty" style="color:var(--text-muted); font-size:13px; padding:12px; text-align:center; <?= !empty($detailSparepart) ? 'display:none;' : '' ?>">Belum ada sparepart ditambahkan (opsional)</p>
            </div>
        </div>
        ...
        <div class="card-panel" style="margin-top:24px; display:flex; justify-content:space-between; align-items:center; padding:20px 24px;">
            ...
            <span id="totalDisplay" style="font-size:24px; font-weight:800; color:var(--primary-red);">Rp <?= number_format($trans['Total_Harga'] ?? 0, 0, ',', '.') ?></span>
        </div>

        <div style="display:flex; gap:12px; margin-top:20px;">
            <button type="submit" class="btn btn-primary"><i class='bx bx-check'></i> Simpan Perubahan</button>
            <a href="/transaksi/<?= $trans['ID_Transaksi'] ?>" class="btn btn-secondary"><i class='bx bx-x'></i> Batal</a>
        </div>
    </form>
</div>
...
// Existing data
const existingServis = <?= json_encode($detailServis) ?>;
const existingSparepart = <?= json_encode($detailSparepart) ?>;

// Pilih kendaraan → tampilkan pelanggan
const selKendaraan = document.getElementById('selKendaraan');
...
// Load pelanggan info on page load
if (selKendaraan.value) {
    selKendaraan.dispatchEvent(new Event('change'));
}

let servisCount = 0;
function addServis(preselectedId = '', preselectedHarga = 0) {
    document.getElementById('servisEmpty').style.display = 'none';
    const idx = servisCount++;
    let opts = '<option value="">-- Pilih Jasa --</option>';
    jasaList.forEach(j => {
        const selected = (preselectedId && j.ID_Jasa == preselectedId) ? 'selected' : '';
        opts += `<option value="${j.ID_Jasa}" data-harga="${j.Harga_Satuan}" ${selected}>${j.Nama_Jasa} — Rp ${Number(j.Harga_Satuan).toLocaleString('id')}</option>`;
    });
    const harga = preselectedHarga || 0;
    ...
    row.innerHTML = `
        ...
        <input type="hidden" name="servis_harga[]" id="servisHarga${idx}" value="${harga}">
        <span class="row-price" id="servisPrice${idx}">Rp ${Number(harga).toLocaleString('id')}</span>
...
function addSparepart(preselectedId = '', preselectedHarga = 0, preselectedJml = 1) {
    ...
    sparepartList.forEach(s => {
        const selected = (preselectedId && s.ID_Sparepart == preselectedId) ? 'selected' : '';
        opts += `<option value="${s.ID_Sparepart}" data-harga="${s.Harga_Jual}" data-stok="${s.Stok_Barang}" ${selected}>${s.Nama_Barang} — Rp ${Number(s.Harga_Jual).toLocaleString('id')} (Stok: ${s.Stok_Barang})</option>`;
    });
    const harga = preselectedHarga || 0;
    const jml = preselectedJml || 1;
    const row = document.createElement('div');
    row.className = 'dynamic-row';
    row.innerHTML = `
        <div class="input-icon-group" style="flex:1;">
        ...
        <div class="input-icon-group" style="width:100px;">
            ...
            <input type="number" name="sparepart_jumlah[]" id="sparepartJml${idx}" value="${jml}" min="1" onchange="sparepartCalc(${idx})" style="width:100%;">
        </div>
        <input type="hidden" name="sparepart_harga[]" id="sparepartHarga${idx}" value="${harga}">
        <span class="row-price" id="sparepartPrice${idx}">Rp ${(harga * jml).toLocaleString('id')}</span>
...
// Load existing servis rows
existingServis.forEach(s => {
    addServis(s.ID_Jasa, s.Harga_Satuan);
});

// Load existing sparepart rows
existingSparepart.forEach(s => {
    addSparepart(s.ID_Sparepart, s.Harga_Satuan, s.Jumlah);
});
</script>
